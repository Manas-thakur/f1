"""Fixture builders for the simulator behaviour suite.

Deliberately the same builder as ``tests/numerics/conftest.py``: pytest gives
each test directory its own ``conftest`` namespace, and a shared helper module
imported across directories would depend on the import mode. The two copies are
kept identical.

The shipped scenarios carry a realistic human reaction delay. A behaviour test
usually needs the input it asks for to be applied when it asks for it, so these
helpers build derived bundles with explicit, documented overrides rather than
adding a second set of fixture files.
"""
