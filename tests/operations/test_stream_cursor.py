...
@pytest.mark.parametrize("cursor", [0, 7, 4096])
def test_an_empty_channel_accepts_any_cursor_instead_of_looping(cursor: int):
    """A restarted control plane holds no history, so it can prove no gap.

    Refusing here is not a repair: the client refetches the snapshot, returns
    with the same cursor, and is refused again for the same reason.
    """
    hub = StreamHub(buffer_size=16, client_queue=32)

    async def scenario() -> bool:
        _, resync = await hub.subscribe("ses-empty-channel", cursor)
        return resync

    assert asyncio.run(scenario()) is False, (
        "an empty channel demanded a resync the client cannot satisfy"
    )
