"""Transactional repository operations that carry lifecycle guarantees.

The atomic selection sequence from ``backend/PERSISTENCE_AND_WORKERS.md`` is
...
def mark_published_batch(db: OrmSession, ids: Sequence[str]) -> int:
    """Mark rows published in one write, without reading them first.

    The drain reads its batch in one unit of work, publishes outside any
    transaction, and marks the delivered rows here. Issuing a write as the
    first statement of that second unit of work matters on SQLite: a
    transaction that reads and only later writes has to promote a snapshot the
    session runtime process has already invalidated, and SQLite refuses that
    promotion outright rather than waiting. ``synchronize_session`` is off for
    the same reason -- the default strategy would issue a SELECT first and put
    the read back in front of the write.
    """
    if not ids:
        return 0
    result = db.execute(
        update(OutboxRecord)
        .where(OutboxRecord.id.in_(list(ids)))
        .values(published_at=datetime.now(UTC), attempts=OutboxRecord.attempts + 1),
