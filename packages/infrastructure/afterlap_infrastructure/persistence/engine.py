...
from afterlap_core.paths import Paths, implementation_root
...
The session runtime owns its own process and writes this store directly, so a
session's decision writes and the control plane's outbox drain are two
processes competing for one write lock. Without a busy timeout the loser fails
immediately with ``database is locked`` rather than waiting its turn, and a
session spools decisions it could have stored.

Forcing every transaction to ``BEGIN IMMEDIATE`` is the other half of the
usual recipe and is deliberately **not** used here: a unit of work that spans
a call into the session runtime would then hold the write lock for the whole
decision, and starve the process actually producing it. Instead no unit of
work reads and later writes across a slow call -- see
``OutboxPublisher.drain_once``, which reads, publishes and marks in three
separate steps.

PostgreSQL needs none of this; it is the price of the zero-server default
store.
"""
...
def ensure_schema(engine: Engine) -> str:
    ...
    api_root = implementation_root() / "apps" / "api"
