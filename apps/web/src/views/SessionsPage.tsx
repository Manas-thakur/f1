...
const COLUMNS: readonly Column<SessionSummary>[] = [
  {
    ...
    cell: (row) => (
      <>
        <Link href={`/sessions/${row.id}/engineer`}>{row.label ?? row.id}</Link>
        <br />
  ...
  { id: 'created', header: 'Created', cell: (row) => row.created_at },
  {
    ...
    cell: (row) => (
      <>
        <Link href={`/sessions/${row.id}/engineer`}>engineer</Link>
        {' · '}
        <Link href={`/sessions/${row.id}/lab`}>lab</Link>
        {' · '}
        <Link href={`/sessions/${row.id}/replay`}>replay</Link>
        {row.mode === 'simulation' ? (
          <>
            {' · '}
            <Link href={`/sessions/${row.id}/driver`}>driver</Link>
          </>
        ) : null}
      </>
    ),
