...
const workspaceDir = "C:/Work/f1/new_plan/presentation";
...
// 2 Engineering question
{
  ...
  footer(s, 2); note(s, "We evaluate the attack and a later retained-position checkpoint. This prevents a policy from earning repeated rewards for passing and being repassed.", ["Project: 06_planning/OPTIMIZATION.md", "Project: 07_learning/ENVIRONMENT_AND_FEATURES.md"]);
}
...
// 4 Circuit comparison
{
  ...
  note(s, "Length is a scale check, not a strategy score. The actual policy receives geometry, conditions and rule context. Monza's official page reports 80 percent full throttle and a 1.1 km main straight. Monaco's page describes narrow streets and difficult overtaking.", ["https://www.formula1.com/en/racing/2026/italy", "https://www.formula1.com/en/racing/2026/monaco", "Project: 17_real_tracks_conditions/season_2026_registry.json"]);
}
...
// 5 Track package
{
  ...
  footer(s, 5); note(s, "The compiler creates a periodic metric representation and aligns it with the latest event documents. A validator, not a manually edited status, decides whether the package can run.", ["Project: 17_real_tracks_conditions/TRACK_DATA_PIPELINE.md", "https://www.fia.com/documents/formula-1"]);
}
...
// 7 Factors
{
  ...
  footer(s, 7); note(s, "The causal graph guides scenario sampling and leakage tests. Weather, grip and tyre state remain correlated. Wind is projected against local track heading rather than treated as one global sign.", ["Project: 17_real_tracks_conditions/RACE_CONDITION_MODEL.md"]);
}
...
// 9 Overtake
{
  ...
  footer(s, 9); note(s, "A completed pass and a retained pass are separate outcomes. Counterfactual opponents react to the changed branch. The policy cannot read simulator labels or future data.", ["Project: 05_estimation/TECHNICAL_SPEC.md", "Project: 17_real_tracks_conditions/RACE_CONDITION_MODEL.md"]);
}
...
// 11 Weights
{
  ...
  footer(s, 11); note(s, "A neural network has no one stable physical importance weight per feature. We use paired interventions, group ablations, Sobol or Morris sensitivity and local actor diagnostics. Each result names the experiment and limits.", ["Project: 17_real_tracks_conditions/FACTOR_AND_INFLUENCE.md"]);
}
...
// 12 Curriculum
{
  ...
  footer(s, 12); note(s, "The curriculum advances only after correctness and baseline gates. Evaluation balances decision opportunities rather than raw lap counts. Natural-frequency weights return for the final report.", ["Project: 17_real_tracks_conditions/RL_TRACK_GENERALISATION.md"]);
}
...
// 14 Evidence
{
  ...
  footer(s, 14); note(s, "The final report uses hierarchical paired intervals over scenarios and training seeds. Failed and withdrawn decisions stay in the denominator. Promotion thresholds are frozen before opening the final test set.", ["Project: 07_learning/SERVING_AND_EVALUATION.md", "Project: 13_validation/TECHNICAL_SPEC.md"]);
}
...
// 15 Product and USP
{
  ...
  footer(s, 15); note(s, "The USP is the auditable chain from real event context to a constrained recommendation and observed execution. The final demonstration must run two actual branches rather than playing authored chart data.", ["Project: 12_design/DESIGN_REVISION_02.md", "Project: 15_presentation/DEMO_RUNBOOK.md", "Project: 17_real_tracks_conditions/README.md"]);
}
