...
# --------------------------------------------------------------------------- #
# 1. deployment changes speed and energy, with one accounting
# --------------------------------------------------------------------------- #
...
def test_attack_deployment_is_faster_at_the_mark_and_costs_battery_energy(circuit: str) -> None:
    ...
    for run in (neutral, attack):
        ledger = run.ledger
        # E_0 - E_final == battery_out - battery_in exactly (the ledger's own closure) ...
        assert abs(ledger.close_error()) < LEDGER_TOLERANCE_J
        # ... and the terminal outflow beyond the auxiliary draw is the integrated DC
        # deployment divided by eta_discharge once: no second conversion, no duplicate.
...
# --------------------------------------------------------------------------- #
# 2. braking regeneration is bounded and grip shrinks it
# --------------------------------------------------------------------------- #
...
def test_lower_grip_harvests_strictly_less_and_the_rest_is_friction_braked(circuit: str) -> None:
    ...
    # Not lost: the braking force is the same whether or not the generator takes a
    # share of it. A car with regeneration disabled on the same wet surface follows
    # the identical speed trajectory; only the routing of the work differs.
    no_regen_cars = {
    ...
    # And the wheel work the generator did not take is what the ledger calls
    # mechanical offer minus harvest, never a negative number.
...
def test_the_grip_law_binds_below_the_max_harvest_ceiling() -> None:
    ...
    # At or below the floor nothing is usable; at the dry reference exactly nothing changes.
...
def test_battery_acceptance_limits_harvest_when_the_car_declares_a_ramp() -> None:
    ...
    # Without a declared ramp there is no thermal harvest limit (current behaviour).
...
# --------------------------------------------------------------------------- #
# 3. event curves: confirmed tightens, unreviewed changes nothing
# --------------------------------------------------------------------------- #
...
def test_a_confirmed_standard_curve_below_the_car_ceiling_reduces_deployment(circuit: str) -> None:
    ...
    # The car starts at 75 m/s = 270 km/h, where the invented curve reads 260 kW.
...
def test_the_scenario_event_id_loads_the_shipped_monza_overlay_lazily() -> None:
    ...
    # The shipped 2026-italy draft is unreviewed and its curves are chart-only.
...
def test_curve_interpolation_and_eligibility_fallback() -> None:
    ...
    # 270 km/h sits between the 250 and 300 km/h rows: 300 + (200 - 300) * 20/50 = 260 kW.
    assert limits.deploy_ceiling_dc_w(75.0) == pytest.approx(260_000.0)
    # Flat extrapolation at both ends; the car ceiling still caps the low-speed end.
    assert limits.deploy_ceiling_dc_w(0.0) == 350_000.0
    assert limits.deploy_ceiling_dc_w(150.0) == pytest.approx(120_000.0)
    # Overtake eligible with a confirmed "no Overtake curve defined" falls back to the
    # standard curve, and the description names the curve actually applied.
    ...
    # A confirmed Overtake curve with rows is used only for an eligible car.
    ...
    # A confirmed empty curve is knowledge ("none defined") and imposes no limit.
