...
def test_ceiling_at_exact_threshold(pack_v1):
    ...
    # Exactly on each declared breakpoint the curve must return the declared
    # value, with no interpolation error at all.
...
def test_ceiling_epsilon_below_and_above_threshold(pack_v1):
    ...
    # Just below 95 m/s the car is still on the steeper leg from 80 m/s.
    ...
    # Just above it the shallower leg towards 110 m/s applies.
    ...
    # The two one-sided values straddle the breakpoint by different amounts,
    # which is exactly why a single "near 95 m/s" tolerance would be wrong.
...
def test_ceiling_extrapolates_flat_outside_range(pack_v1):
    curve = season_curve(pack_v1)
    # Below the first breakpoint the first value is held; above the last, the
    # last value is held. Nothing is extrapolated along a gradient.
    assert curve.ceiling_w(0.0) == 350_000.0
    assert curve.ceiling_w(110.0) == 0.0
...
def test_ceiling_is_continuous_at_every_breakpoint(pack_v1):
    curve = season_curve(pack_v1)
    delta = 1.0e-6
    # Steepest declared leg, computed from the breakpoints themselves.
    steepest = max(
...
def test_absolute_ceiling_and_curve_intersect_at_declared_bus(pack_v1):
    ...
    # The recharge allowance lives on a different bus and must never be mixed in.
    ...
    for speed_mps, expected_w in ((50.0, 350_000.0), (100.0, 100_000.0), (110.0, 0.0)):
        ...
        # min(absolute ceiling, curve at this speed), then the derate factor of 1.0.
...
def test_event_sector_curve_overrides_the_season_curve(pack_v1):
    # Event sector curve: (0, 300 kW) (70, 300 kW) (95, 120 kW) (110, 0 kW).
    ...
    # Midpoint of the 70 -> 95 leg: 300 kW - half of the 180 kW drop.
    ...
    # 2.5 m/s past the 80 m/s breakpoint on the -200000/15 leg.
