...
A conditions document declares either::
    ...
    source: openf1
    session_key: 9912            # tape cached at artifacts/conditions/<id>.json

or::

    source: synthetic
    synthetic:                   # explicit SI constants held for duration_s
...
def tape_cache_path(conditions_id: str, paths: Paths | None = None):
...
def synthetic_tape(config: ConditionsConfig) -> ConditionsTape:
    ...
    assert constants is not None  # validated
...
def load_conditions(
    ...
    assert config.session_key is not None  # validated
