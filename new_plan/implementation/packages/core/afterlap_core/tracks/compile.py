...
def fit_periodic_spline(
    ...
    # Fold the k wrap-around columns onto the first k periodic basis functions.
    wrap = hstack([b_ext[:, m : m + k], csr_matrix((n, m - k))], format="csr")
    b_per = (b_ext[:, :m] + wrap).tocsr()
    # Circular second differences.
    d2 = lil_matrix((m, m))
    for i in range(m):
        d2[i, i] = 1.0
        d2[i, (i + 1) % m] = -2.0
    ...
    *,
    ...
    for bandwidth in candidates_m:
        ...
        for k in range(folds):
            ...
            # Fold fits start at their own parameter origin; align by nearest point on the full grid.
...
def register_and_aggregate(laps: list[LapGeometry]) -> tuple[np.ndarray, RegistrationReport]:
    ...
    L = ref_line.length_3d_m
    n_bins = max(8, round(L / REGISTRATION_BIN_M))
    ...
    med_lat = _periodic_fill(centres, med_lat, L)
    med_dz = _periodic_fill(centres, med_dz, L)
    base = np.column_stack(
        [np.interp(centres, ref_line.s_m, ref_line.xyz_m[:, k], period=L) for k in range(3)]
    )
    cos_yaw = np.interp(centres, ref_line.s_m, np.cos(ref_line.yaw_rad), period=L)
    sin_yaw = np.interp(centres, ref_line.s_m, np.sin(ref_line.yaw_rad), period=L)
...
def compile_track(manifest_path: Path, paths: Paths) -> TrackPackage:
    ...
    # Scale inference from the raw polyline of every lap (metres are unknown until here).
    ...
    # The elevation channel gets its own bandwidth, selected on vertical residual.
    ...
    # Timing line: median of every lap's interpolated boundary crossing, projected onto the line.
    ...
    # Residuals of every clean lap against the final line.
    ...
    # Sector boundaries from timing durations (not from an FIA map).
    ...
    # Raw versus smoothed curvature, for the record.
