...
class Interaction:
    ...
    def present(self) -> bool:
        ...
        return (
            self.ahead_id is not None
            or self.behind_id is not None
            or self.recovery > 0
            or self.cooldown > 0
        )
...
class _BasePolicy:
    ...
    def _update_interaction(self, observation: Observation) -> Interaction:
        ...
        # An order change is the interaction worth remembering: a car that was
        # behind and is now ahead has passed this one, and the reverse.
    ...
    def _decide(self, observation: Observation, interaction: Interaction) -> DriverAction:
        ...
        if ahead is None or not has_margin:
            ...
            if gap > self.release_gap_s:
                # The move did not stick and the car ahead has pulled clear.
    ...
    def _decide(self, observation: Observation, interaction: Interaction) -> DriverAction:
        ...
        if interaction.just_lost_a_position and interaction.ahead_id is not None:
            ...
            return DriverAction(
                ...
                pace_scale=self.pace_scale,
                # Mirror of the defensive line: the car that just went by is on
                # the defensive side, so the recovery line is the other one.
                target_lateral_d_m=-self.lateral_offset_m,
                issued_at_s=observation.delivered_at_s,
                label=f"{self.kind}:{self.state}",
            )
