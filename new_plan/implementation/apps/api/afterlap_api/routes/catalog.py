...
def _overlay_ids(track_id: str, paths: Paths) -> tuple[str, ...]:
    ...
    if not root.is_dir():
        return ()
    # A16-3 writes ``<event>.extraction.json`` sidecars and
    # ``<event>.superseded-<sha>.json`` history beside the overlays; both carry
    # a second dot and are not overlays.
...
async def list_tracks(request: Request) -> TrackListResponse:
    ...
    # Union, not intersection: a package with no registry entry is as much a
    # reporting failure as a registry entry with no package.
