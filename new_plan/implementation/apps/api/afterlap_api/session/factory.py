...
def resolve_artefacts(
    ...
    **override** the scenario document's own values. The override is recorded
    ...
    # The readiness and hash checks run *before* the bundle is built, so a
    # package below geometry_validated is refused with the readiness reason
    # rather than with whatever ``load_track`` happens to raise first.
...
def build_manifest(
    ...
    *,
    ...
    capability = simulator_session_capability(
        ...
        # The circuit notes carry the real_circuit_synthetic_energy label, the
        # unknown-corridor consequence and the unreviewed-overlay consequence.
        # They belong on the source capability so they survive into the stored
        # manifest and every export of it, not only into a live response.
