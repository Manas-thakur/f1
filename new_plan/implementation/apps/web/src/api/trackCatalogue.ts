/**
 * Reads for the real-circuit catalogue: tracks, compiled centrelines,
 * condition tapes and scenario documents.
 *
 * This is **not** a second API client and not a second wire format. It uses
 * the same base path (`/api/v1`), the same typed-error decoding and the same
 * `RequestOptions` contract as `src/api/client.ts`; it exists only because the
 * shared client is owned by another agent and does not expose these routes.
 * Folding these four methods into `ApiClient` is a named integration action in
 * `handoffs/A16-9.md`.
 *
 * The wire shapes are the Pydantic response models in
 * `apps/api/afterlap_api/routes/catalog.py` — `TrackListResponse`,
 * `CentrelineResponse`, `ConditionsListResponse`, `ScenarioListResponse` —
 * read field for field, not guessed. The generated contract bundle does not
 * emit them (they are route-local models, not domain contracts), so they are
 * declared here; regenerating them into `packages/contracts` and importing
 * them instead is the other named integration action.
 *
 * Nothing here has a fallback. Every response passes a decoder that either
 * returns the value or returns a problem string naming the field that was
 * missing or the wrong type; the views render that as an explicit unavailable
 * state naming the route. No shipped list, cached hash, image asset or traced
 * outline ever stands in for a route that failed.
 */
...
/** `afterlap_core.tracks.package.ReadinessStatus`. */
...
/** Ladder order from TRACK_REGISTRY_2026.md. `rejected` sits below the ladder. */
...
/** `catalog.TrackSummary`: registry identity joined with the compiled package. */
export interface TrackCatalogueEntry {
  ...
  /**
   * The rung to judge this circuit by: the package's own validated status when
   * a package exists, otherwise the registry rung. Null when neither parses.
   */
  readonly readiness: ReadinessRung | null;
  /** Verbatim, for display when it did not parse as a rung. */
  readonly readiness_reported: string | null;
  /** The package's validated status, absent when no package is compiled. */
...
/** `catalog.CentrelineResponse`. Coordinates are metres in the package frame. */
export interface CentrelineResponse {
  ...
  /** Samples in the compiled package, before the server strided. */
...
/** `catalog.ConditionsSummary`. */
export interface ConditionsCatalogueEntry {
  readonly conditions_id: string;
  readonly description: string | null;
  readonly source: string | null;
  /** False when the frozen tape is absent; `unavailable_reason` says why. */
...
/** `catalog.ConditionsListResponse`. */
...
export interface ScenarioCatalogueEntry {
  ...
  /** `real_circuit_synthetic_energy` when the scenario resolves a package. */
  readonly run_label: string | null;
  readonly unavailable_reason: string | null;
}

/** `catalog.ScenarioListResponse`. */
...
function decodeTrackEntry(raw: unknown, index: number): DecodeResult<TrackCatalogueEntry> {
  ...
  // The package's validated status wins: a registry rung is identity only,
  // and a compiled package that failed validation must not be judged by it.
...
/** Thrown when a route answered 200 with a body the decoder refused. */
...
export class TrackCatalogueClient {
  ...
  /** Every registered circuit joined with the compiled packages on disk. */
  listTracks(options: RequestOptions = {}) {
    return this.read('/tracks', decodeTrackList, options);
  }

  /**
   * The compiled centreline, strided server-side.
   *
   * `stride_m` is honoured in index space at the package's own sample
   * spacing, and the response repeats `package_hash` and `arrays_sha256` so a
   * viewer can prove the coordinates came from the geometry a session ran. No
   * client-side resampling or smoothing is applied to what comes back.
   */
  ...
  /**
   * Every conditions document, resolved offline.
   *
   * The route takes no circuit filter — a tape is a document, and its binding
   * to a venue is the operator's judgement plus the tape's own altitude and
   * session key — so none is sent and none is implied.
   */
  listConditions(options: RequestOptions = {}) {
    return this.read('/conditions', decodeConditionsList, options);
  }

  /** Scenario documents with the circuit and tape each one resolves to. */
...
/**
 * One sentence naming the route and the failure, for an unavailable state.
 *
 * `not_found` on the centreline path means the circuit has no compiled
 * package or no compiled arrays; on the list paths it usually means the
 * router is not mounted. The text says which, and never implies data exists.
 */
