 ...
 * The circuit map.
 *
 * Inline SVG drawn from the coordinates `GET /api/v1/tracks/{id}/centreline`
 * returned — the compiled package's own metric centreline, downsampled by the
 * server. There is no traced artwork, no image asset, no smoothing and no
 * client-side resampling: the polyline is exactly the points the control plane
 * sent, projected once into the view box.
 *
 * The package hash is printed next to the drawing so a viewer can check that
 * the shape belongs to the package the session claims, and the OpenF1
 * attribution is printed with it because this geometry is a driven line under
 * CC BY-NC-SA 4.0 (D-11).
 */
...
/** Padding inside the view box, in the same metres as the coordinates. */
const PAD_FRACTION = 0.04;

export interface CircuitMapProps {
  readonly centreline: CentrelineResponse;
  readonly displayName?: string | null;
  /** Rung shown with the drawing, so a map never travels without its status. */
...
/**
 * Project metres to SVG user units.
 *
 * The y axis is negated once, because SVG y grows downward and an ENU
 * northing grows upward; without it the circuit is drawn mirrored. Nothing
 * else is transformed, so distances stay proportional in both axes and the
 * scale bar below is honest.
 */
function project(x: readonly number[], y: readonly number[]): Projection | null {
  ...
  // A round scale bar of roughly one fifth of the wider axis.
    ...
    <div className={styles.mapFrame}>
      <div className={styles.mapSurface}>
        ...
        >
          ...
          />
          {/*
            The view box is in metres, so a CSS pixel font-size would render
            the label a few metres tall — invisible on a 7 km circuit. Size it
            as a fraction of the drawing instead, and let the browser scale it
