/**
 * `Circuit and conditions` — the laboratory's first configuration step.
 *
 * Three live reads, no shipped lists and no fallbacks:
 *
 *   GET /api/v1/tracks                          the circuit rows and their readiness
 *   GET /api/v1/tracks/{id}/centreline?stride_m the coordinates the map is drawn from
 *   GET /api/v1/conditions                      the condition tapes
 *
 * When a route is missing or errors, the section that needed it renders an
 * explicit unavailable state naming the route and the reason. It never falls
 * back to a hardcoded circuit list, a cached hash, an image asset or a
 * placeholder outline, and it never lets a circuit be selected on the
 * strength of an assumption about its readiness.
 */
...
export interface CircuitConfigurationProps {
  /** Test seam. Production uses the module-level catalogue client. */
