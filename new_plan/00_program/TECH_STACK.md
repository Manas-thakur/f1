...
This is the implementation decision, not a list of alternatives. All production paths are below `new_plan/implementation/`. A01 owns dependency selection and locks. Numeric major lines below are compatibility targets, not claims of latest versions; freeze released compatible patch versions only after the G0 installation spike.
...
configs/ infra/ tests/ artifacts/ handoffs/
```
