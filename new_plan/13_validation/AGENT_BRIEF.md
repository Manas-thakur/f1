# A13 — independent evaluation: ready-to-paste agent brief

Implement the independent evaluation module for AFTERLAP using this specification package. This is implementation work, not a request for another plan.
...
All production paths are below **new_plan/implementation/**. Own only: packages/core/evaluation/; tests/acceptance/. Ignore all existing contents outside new_plan. Do not alter other workers' modules, root dependency files, schemas, database migrations or router registration without coordinator integration. A01 is the coordinator exception for its declared responsibilities. Do not overwrite the documentation mockups with production code.

## Dependencies

module interfaces; coordinator test manifests. Use coordinator-provided contract fixtures until dependencies are integrated. Missing dependencies must return explicit unavailable states, not fake successful behaviour. Submit contract changes in `implementation/handoffs/A13-contract-proposal.md` and continue independent work.

## Deliverables

Independent gates, paired benchmarks, calibration, ablation and failure reports. Include public interfaces, deterministic fixtures, meaningful tests, diagnostics, failure paths and source/provenance notes. Implement all acceptance cases in the module specification. Validate the shared contract vectors. No real-F1 actuation, hidden simulator-truth access, fabricated measurements or live model exploration.
...
7. Write `implementation/handoffs/A13.md` with paths, commands/results, assumptions, unresolved issues and required integration actions.
