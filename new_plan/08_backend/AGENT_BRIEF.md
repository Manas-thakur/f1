...
All production paths are below **new_plan/implementation/**. Own only: apps/api/; workers/session_runtime/; tests/backend/. Ignore all existing contents outside new_plan. Do not alter other workers' modules, root dependency files, schemas, database migrations or router registration without coordinator integration. A01 is the coordinator exception for its declared responsibilities. Do not overwrite the documentation mockups with production code.

## Dependencies

01_contracts plus runtime module interfaces. Use coordinator-provided contract fixtures until dependencies are integrated. Missing dependencies must return explicit unavailable states, not fake successful behaviour. Submit contract changes in `implementation/handoffs/A08-contract-proposal.md` and continue independent work.

## Deliverables

Routes, session authority, persistence, lifecycle, streams and recovery. Include public interfaces, deterministic fixtures, meaningful tests, diagnostics, failure paths and source/provenance notes. Implement all acceptance cases in the module specification. Validate the shared contract vectors. No real-F1 actuation, hidden simulator-truth access, fabricated measurements or live model exploration.
...
7. Write `implementation/handoffs/A08.md` with paths, commands/results, assumptions, unresolved issues and required integration actions.
