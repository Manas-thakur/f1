...
Training uses CPU Stable-Baselines3 PPO. The default setup uses a multilayer perceptron, a discount factor of `0.996672`, ten optimizer epochs per cycle, and seeded simulation episodes. Each requested training cycle is followed by evaluation on a held-out seed range. The best evaluation checkpoint is saved whenever mean reward improves.
