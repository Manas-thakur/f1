...
All production paths are at the repository root. Own only: packages/core/planning/; tests/planning/. Ignore files outside this repository. Do not alter other workers' modules, root dependency files, schemas, database migrations or router registration without coordinator integration. A01 is the coordinator exception for its declared responsibilities. Do not overwrite the documentation mockups with production code.

## Dependencies

simulation, rules, estimation. Use coordinator-provided contract fixtures until dependencies are integrated. Missing dependencies must return explicit unavailable states, not fake successful behaviour. Submit contract changes in `docs/handoffs/A06-contract-proposal.md` and continue independent work.
...
7. Write `docs/handoffs/A06.md` with paths, commands/results, assumptions, unresolved issues and required integration actions.
