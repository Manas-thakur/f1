...
| Item | State |
|---|---|
| **A16 real circuits and conditions** | **not built.** `BUILD_WITH_AGENTS.md` was revised mid-build to require it and `docs/tracks/` holds 397 lines of specification plus a track-package schema, a season registry and a Monza example. No pipeline, geometry compiler, event overlay or condition model exists. The product does not meet its current specification. |
