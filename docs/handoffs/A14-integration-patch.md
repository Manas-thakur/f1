...
`operations/TECHNICAL_SPEC.md` lists graceful shutdown as an acceptance case
...
def _remember_state(request: Request, session_id: str, tick) -> None:  # type: ignore[no-untyped-def]
    ...
    # hide a stale feed, which operations/TECHNICAL_SPEC.md calls out.
...
```python
    ...
    # observable, and operations/TECHNICAL_SPEC.md asks for it by name.
...
`operations/TECHNICAL_SPEC.md`: *"A healthy HTTP server with stale telemetry
