...
All production paths are at the repository root. Own only: apps/api/ (Python CLI and session runtime); workers/; tests/backend/. Next.js owns public HTTP. Ignore files outside this repository. Do not alter other workers' modules, root dependency files, schemas, database migrations or router registration without coordinator integration. A01 is the coordinator exception for its declared responsibilities. Do not overwrite the documentation mockups with production code.
