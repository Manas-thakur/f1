...
Base `/api/v1`. Next.js is the public HTTP origin. Route handlers spawn `python -m afterlap_api.cli request` and `python -m afterlap_api.cli stream`. Python has no HTTP server. Envelope JSON is unchanged.
