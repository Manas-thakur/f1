...
ENV AFTERLAP_RACE_UPSTREAM=http://127.0.0.1:18761
RUN bun run --filter @afterlap/web build

FROM simulator AS runtime
COPY --from=node:22-bookworm-slim /usr/local/bin/node /usr/local/bin/node
COPY --from=web-build /src/apps/web/.next/standalone /web/
COPY --from=web-build /src/apps/web/.next/static /web/apps/web/.next/static
COPY --from=web-build /src/apps/web/public /web/apps/web/public
ENV PATH="/app/.venv/bin:${PATH}" HOSTNAME=0.0.0.0
