...
`RaceEnv`, the JSONL generator, PPO collection and the WebSocket server all advance `RaceSession`. It clears `world.policies`; changing `simulation/policies.py` policy classes alone cannot fix live racecraft. The session calls `Racecraft.react` at 0.1 simulated seconds. A seeded periodic profile supplies each car's moving lateral target. The engine executes delayed commands and integrates physical forces at the configured step.
...
The observed gap is advanced by relative speed times sensor age plus command reaction delay. Local gaps wrap into half a lap in either direction. Candidate sides use vehicle width plus preferred clearance, not fixed lane centres. In default overlap mode, a new pass requires positive closing speed but neither footprint clearance nor spare cornering capacity. Cars can occupy the same space, and physical overlap does not block pass completion. Optional terminate mode checks all nearby rivals across the swept lateral corridor and predicted longitudinal interval. Disabling corner overtakes adds the conservative tactical gate based on known track curvature and observed grip.
...
Terminate mode follows with `a_follow = (0.45 * (net_gap - v * headway) - 1.2 * closing_speed) / (1 + headway)`. Coefficients carry the implicit SI scaling of this synthetic controller. The requested acceleration is also bounded by a relative stopping-distance term `4 - closing_positive² / (2 * net_gap)`. Default overlap mode deliberately omits this traffic braking. The engine translates the resulting ceiling to wheel force, with drag/rolling/grade compensation, actual deployment fraction, tyre/brake limits and the existing battery ledger. It never sets speed or position directly.

Lateral targets advance at the driver's bounded rate. Smoothed curvature creates outside, apex and exit regions, while low-frequency seeded harmonics vary the path per car and episode. The profile is periodic and clipped to the usable corridor. The plant still uses first-order line tracking. This does not model yaw inertia, lateral slip, load transfer or the complete force cost of a lane change. Default overlap behavior is an intentional visualization and training simplification, not a collision-avoidance model. Cost is linear in observed rivals per driver decision, with constant-size forecasts.
...
The combined tyre envelope, battery thermal response, regeneration and numerical checks remain active. Electrical deployment is further limited to the propulsion headroom left by combustion power, the combined shaft-power ceiling and the longitudinal tyre envelope. Depletion or thermal shutdown latches Boost into Harvest until a non-boost request rearms it, preventing recovered energy from cycling through a continuously held Boost request. No placeholder settings are counted as implemented mechanisms.
